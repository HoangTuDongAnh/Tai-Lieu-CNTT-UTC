...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CONTEXTMENU:
    {
        ...
        TrackPopupMenu(hsubmenu, TPM_RIGHTBUTTON, LOWORD(lParam), HIWORD(lParam), 0, hWnd, NULL);
        DestroyMenu(hmenu);
	}
    break;
    case WM_COMMAND:
        {
            ...
            case ID_LANGUAGE_VIETNAMESE:
            {
                ...
                HMENU hNew = LoadMenu(hInst, MAKEINTRESOURCE(IDR_MENU2)); 

                SetMenu(hWnd, hNew); 
                if (hCurrent) DestroyMenu(hCurrent);
                DrawMenuBar(hWnd); 
            }
            break;
            ...
            {
                HMENU hCurrent = GetMenu(hWnd);
                HMENU hNew = LoadMenu(hInst, MAKEINTRESOURCE(IDR_MENU1));
